...
int device_enable(sys_ctrl_ctx_t *ctx, clk_id_t clk_id, reset_id_t rst_id) {
    ...
    printf("current clock source = %u\n", source);

    freq_t freq = get_freq(ctx, clk_id);
    printf("clock frequency before source set = %" PRIu64 "\n", freq);

    // source = set_clock_source(ctx, clk_id, 102);
    // printf("newly set clock source = %u\n", source);

    // freq = set_freq(ctx, clk_id, 408000000);
    // printf("clock frequency after set = %" PRIu64 "\n", freq);

    // freq = get_freq(ctx, clk_id);
    // printf("clock frequency after source set = %" PRIu64 "\n", freq);

    // freq = set_freq(ctx, clk_id, 204000000);
    // printf("clock frequency after set = %" PRIu64 "\n", freq);

    // freq = get_freq(ctx, clk_id);
    // printf("clock frequency after source set = %" PRIu64 "\n", freq);

    // freq = set_freq(ctx, clk_id, 102000000);
    // printf("clock frequency after set = %" PRIu64 "\n", freq);

    // freq = get_freq(ctx, clk_id);
    // printf("clock frequency after source set = %" PRIu64 "\n", freq);

    // freq = set_freq(ctx, clk_id, 1843200);
    // printf("clock frequency after set = %" PRIu64 "\n", freq);
